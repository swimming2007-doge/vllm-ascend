...
class Gemma4MTPAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Ascend npu_rotary_embedding kernel requires a non-None key.
        # Key output is unused — KV comes from target cache via sharing.
        dummy_key = torch.empty_like(q)
        q, _ = self.rotary_emb(positions, q, dummy_key)
...
class Gemma4MTP(nn.Module):
    ...
    def compute_logits(
        ...
        if self.masked_embedding is not None:
            logits = self.masked_embedding.compute_logits(
                hidden_states,
